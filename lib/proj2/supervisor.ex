  ...
  def initialize_algorithm(actors, topology, totalNodes, algorithm) do
    ...
    neighbors =
      case topology do
        ...
        _ ->
          IO.puts(
            "Please use one of the full | line | rand2D | 3Dtorus | honeycomb | randhoneycomb as topology"
          )
    ...
    for {pid, neighbors_pid} <- neighbors do
      ...
      Proj2.Client.set_neighbors(pid, neighbors_pid)
    end

    # start time of convergence
    ...
    IO.puts(
      "Convergence Time: " <>
        to_string(System.monotonic_time(:millisecond) - start_time) <> " milliseconds"
    )
