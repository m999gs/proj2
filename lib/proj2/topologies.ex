defmodule Proj2.Topologies do
  ...
  def creating_line_network(actors) do
    ...
    actor_index =
      Stream.with_index(actors)
      |> Enum.reduce(%{}, fn {actor, index}, acc -> Map.put(acc, index, actor) end)
  ...
  def creating_rand2D_network(actors) do
    ...
    Enum.reduce(0..(n - 1), %{}, fn i, acc ->
      ...
      neighbors =
        Enum.reduce(remainingKeys, %{}, fn key, acc ->
  ...
  def creating_3Dtorus_network(actors) do
    ...
    Enum.reduce(0..(n - 1), %{}, fn i, acc ->
            ...
            j == 1 ->
              if i - number >= lowerlimit do
                ...
                Map.put(
                  acc,
                  j,
                  Enum.reduce(i..n, i, fn _, acc ->
                    if acc < upperlimit && acc < n do
                      acc + number
                    else
                      acc
                    end
            ...
            j == 2 ->
              if i + number < upperlimit && i + number < n do
                ...
                Map.put(
                  acc,
                  j,
                  Enum.reduce(i..0, i, fn _, acc ->
                    if acc >= lowerlimit do
                      acc - number
                    else
                      acc
                    end
            ...
            j == 4 ->
              if rem(i + 1, number) != 0 && i + 1 < n do
                ...
                Map.put(
                  ...
                  j,
                  if i == n - 1 do
                    Enum.reduce(lowerlimit..n, lowerlimit, fn _, acc ->
                      if acc < n do
                        acc + number
                      else
                        acc
      ...
      # Mapping the neighbors after assigning their values.
  ...
  def creating_honeycomb_network(actors, topology) do
    ...
    Enum.reduce(0..(n - 1), %{}, fn i, acc ->
      neighbors =
        Enum.reduce(1..3, %{}, fn j, acc ->
          cond do
            # odd top addition
            j == 1 && rem(i, 2) != 0 && i - number >= 0 ->
              Map.put(acc, j, i - number)

            # even bottom addition
