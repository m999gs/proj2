defmodule Proj2.Topologies do
  # each actor connected to every other actor except himself
  ...
  #all the actors are connected in the line topology  
  ...
  def creating_rand2D_network(actors) do
    n = length(actors)
    actor_index = Stream.with_index(actors) |> Enum.reduce(%{}, fn({actor , index}, acc) -> Map.put(acc, index, actor) 
  end)
  ...
  def creating_3Dtorus_network(actors) do
    ...
    Enum.reduce(0..n-1, %{}, fn i,acc ->
        ...
        neighbors = Enum.reduce(1..6, %{}, fn (j, acc) ->
            # Get 6 neighbors
            #meaning of top, bottom, left, right, front, back is mentioned in my github repository
            #File name "Dos topology making notes.pdf"
            cond do
            (j == 1) ->   #top
